...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        for ($i = 1; $i <= $countnumber; $i++) {
            if (isset($_POST["item_name"][$i - 1]) && isset($_POST["sale_price"][$i - 1]) && isset($_POST["number"][$i - 1])){
                $item_name = $_POST["item_name"][$i - 1];
                $item_price = $_POST["sale_price"][$i - 1];
                $number = $_POST["number"][$i - 1];

                if ($type == "sales") {
                    if (isset($_POST["clientSelect"][0])){
                        $clientSelect=$_POST["clientSelect"][0];
                        if ( $clientSelect=="client"){
                            if (isset($_POST["client_name"][0]) && isset($_POST["invoiceNumber"][0])&& isset($_POST["total_price"][$i - 1])){
                    ...
                    if (isset($_POST["purchase_price"][$i - 1])&& isset ($_POST["sale_price_client"][$i -1])){
        ...
        if ($type == "sales"){
            ...
            if ( $clientSelect =="client"){
                for ($i = 1; $i <= $countnumber; $i++) {
                    if (isset($_POST["total_price"][$i - 1])){
                        $total_price = $_POST["total_price"][$i - 1] ;
